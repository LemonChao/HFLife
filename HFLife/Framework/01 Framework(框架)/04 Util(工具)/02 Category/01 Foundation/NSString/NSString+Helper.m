...
#pragma mark 护照验证
- (BOOL) isPassport {
    NSString *regex = @"^([a-zA-z]|[0-9]){5,17}$";
...
#pragma mark 军官证验证
- (BOOL) isOfficer {
    NSString *regex = @"^[\u4E00-\u9FA5](字第)([0-9a-zA-Z]{4,8})(号?)$";
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
    BOOL isRe = [predicate evaluateWithObject:self];
    return isRe;
}
...
-(BOOL) isValidateIdentityCard {
    if ([self isValidateIdentityCardForC]) {
        return YES;
    }
    //    return [self isHKTCard];
    return NO;
    
}

-(BOOL) isValidateIdentityCardForC
{
...
- (BOOL) isHKTCard {
    ...
    NSString *regex = @"^[A-Z]{1,2}[0-9]{6}[(][0-9A][)]$";//香港
    
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
    BOOL isRe = [predicate evaluateWithObject:self];
    
    regex = @"^[1|5|7][0-9]{6}[(][0-9][)]$";//澳门
    predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
    BOOL isRe2 = [predicate evaluateWithObject:self];
    
    regex = @"^[A-Z][0-9]{9}$";//台湾
    predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
    BOOL isRe3 = [predicate evaluateWithObject:self];
    
    if (isRe)
    {
        return YES;
        NSString *chekStr = @"";
        NSArray *weightingArray = @[@"8", @"7", @"6", @"5", @"4", @"3", @"2"];
        
        NSInteger sum = 0;//保存前位各自乖以加权因子后的总和
        
        for (int i = 0; i < weightingArray.count; i++) {//将前位数字和加权因子相乘的结果相加
            NSString *subStr = [self substringWithRange:NSMakeRange(i, 1)];
            if (i == 0) {
                sum += ([subStr characterAtIndex:0] - 'A' + 1) * [weightingArray[i] integerValue];
            }else {
                sum += [subStr integerValue] * [weightingArray[i] integerValue];
            }
        }
        ...
        if (chekNum == 0) {
            ...
            chekStr = [NSString stringWithFormat:@"%d",11 - chekNum];
        }
        NSString *idCardChek = [self substringWithRange:NSMakeRange(self.length - 2, 1)]; //获取身份证最后一位校验数字
        if ([idCardChek isEqualToString:chekStr]) {
            return YES;
        }
        return NO;
...
+(NSString *)EncodeidCard:(NSString *)idCard{
    ...
    return [NSString stringWithFormat:@"%@******%@", [idCard substringToIndex:4],[idCard substringFromIndex:10]];;
}
#pragma mark - 银行卡号加密
/**
 *银行卡号加密
...
-(NSString *)EncodeAlipay{
    NSString *Alipay=self;
    if (Alipay.length>4) {
        NSString *placeStr = @"**********************";
        NSInteger toCutIndex = Alipay.length - 4;
        if (toCutIndex > placeStr.length) {
            toCutIndex = placeStr.length;
        }
        placeStr = [placeStr substringToIndex:toCutIndex];
        Alipay=[Alipay stringByReplacingCharactersInRange:NSMakeRange(0,Alipay.length-4) withString:placeStr];
    }
    return Alipay;
}
...
+ (NSString *)amountProcessing:(NSString *)string{
    ...
    for (NSString *str in stringArray) {
        if (string1.length == 0) {
            ...
            string1 = MMNSStringFormat(@"%@,%@",string1,str);
        }
    }
    
    string1 = [NSString reverseWordsInString:string1];
    if ([string rangeOfString:@"."].location != NSNotFound) {
        return MMNSStringFormat(@"%@.%@",string1,array[1]);
    }else{
        return MMNSStringFormat(@"%@.00",string1);
    }
