...
{
    HRBaseViewController *baseViewController = [super allocWithZone:zone];
    WEAK(weakSelf)
    __weak typeof(baseViewController)weakBaseViewController =  baseViewController;
...
//        @strongify(baseViewController)
        [baseViewController hr_configUI];
        [baseViewController hr_bingViewModel];
    }];
    
    [[baseViewController rac_signalForSelector:@selector(viewWillAppear:)] subscribeNext:^(id x) {
        
//        @strongify(baseViewController)
        [baseViewController hr_refreshRequest];
    }];
...
- (void)rightItems:(NSArray <HRBarItem *>*)items actions:(void(^)(NSInteger index))actions{
    self.rightItemActionBlock = actions;
    NSMutableArray *barButtonItems = [NSMutableArray array];
    [items enumerateObjectsUsingBlock:^(HRBarItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithCustomView:obj];
        obj.tag = idx;
