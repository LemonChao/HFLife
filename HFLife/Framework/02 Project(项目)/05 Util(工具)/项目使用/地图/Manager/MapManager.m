...
-(MAAnnotationView *)mapView:(MAMapView *)mapView viewForAnnotation:(id<MAAnnotation>)annotation{
    ...
    if ([annotation isKindOfClass:[MAUserLocation class]]) {
        ...
        //点击大头针显示的左边的视图
        UIImageView *imageV = [[UIImageView alloc]initWithFrame:CGRectMake(WidthRatio(19), 0, WidthRatio(82), WidthRatio(82))];
        imageV.contentMode = UIViewContentModeScaleAspectFill	;
        MMViewBorderRadius(imageV, WidthRatio(10), 0, [UIColor clearColor]);
        if (!self.destinationImgName) {
            if (![NSString isNOTNull:self.destinationImgURL]) {
                [imageV sd_setImageWithURL:[NSURL URLWithString:self.destinationImgURL] placeholderImage:MMGetImage(@"目的地")];
            }else{
                imageV.image = MMGetImage(@"目的地");
            }
        }else{
...
//        点击大头针显示的右边视图
        UIButton *rightButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, WidthRatio(127), 50)];
//        rightButton.backgroundColor = [UIColor grayColor];
        rightButton.titleLabel.font = [UIFont systemFontOfSize:WidthRatio(20)];
        ...
        [rightButton addSubview:lin];
        [lin mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(rightButton.mas_left);
            make.centerY.mas_equalTo(rightButton.mas_centerY);
            make.width.mas_equalTo(WidthRatio(2));
            make.height.mas_equalTo(HeightRatio(78));
        }];
...
-(void)navBtnClick{
    if (self.navigationPath) {
        self.navigationPath(distinateCoor.latitude, distinateCoor.longitude);
    }else{
