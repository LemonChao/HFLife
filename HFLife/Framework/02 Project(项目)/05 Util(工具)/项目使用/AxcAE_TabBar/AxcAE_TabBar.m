...
- (void)configuration{
    ...
    [self addSubview:self.maskBackgroundImageView];
    self.maskBackgroundImageView.backgroundColor = [UIColor clearColor];
...
- (void)switch_tabBarItemIndex:(NSInteger )index WithAnimation:(BOOL )animation{
    ...
    if (item.itemModel.isRepeatClick) { // 允许重复点击触发动画
        ...
        if (![lastItem isEqual: item]) { // 不是上次点击的
            ...
            //
            if (self.delegate && [self.delegate respondsToSelector:@selector(axcAE_RepeatClickTabBar:selectIndex:)]) {
                [self.delegate axcAE_RepeatClickTabBar:weakSelf selectIndex:index];
            }
...
//    self.backgroundImageView.frame = self.frame;
    self.backgroundImageView.frame = CGRectMake(0, 0, SCREEN_WIDTH, 49.0f);
    self.maskBackgroundImageView.frame = CGRectMake(0, 49.0f, SCREEN_WIDTH, 44.0f);
...
- (UIImageView *)maskBackgroundImageView{
    if (!_maskBackgroundImageView) {
        _maskBackgroundImageView = [UIImageView new];
    }
    return _maskBackgroundImageView;
}
