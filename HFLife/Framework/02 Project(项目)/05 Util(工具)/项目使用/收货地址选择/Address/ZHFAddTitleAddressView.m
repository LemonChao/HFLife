...
-(UIView *)initAddressView{
    ...
    self.resultArr = [NSJSONSerialization JSONObjectWithData:resData options:NSJSONReadingMutableLeaves error:nil];
    //------到这里
...
-(void)setDefaultValuesTitles:(NSArray *)titles TitleIDMarr:(NSArray *)titleIDMarr{
	for (int i = 0 ; i < titleIDMarr.count; i++) {
            //2.修改标题
        [self.titleMarr replaceObjectAtIndex:i withObject:titles[i]];
        [self.titleIDMarr addObject:titleIDMarr[i]];
            //请求网络 添加市区
        [self getAddressMessageDataAddressID:i+2 provinceIdOrCityId:titleIDMarr[i]];
        if (i>=3) {
...
-(void)tapBtnAndcancelBtnClick{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        if ([self.delegate1 respondsToSelector:@selector(cancelBtnClick:titleID:)]) {
             [self.delegate1 cancelBtnClick:titleAddress titleID:titleID];
        }
        if ([self.delegate1 respondsToSelector:@selector(cancelBtnClick:titleIDs:)]) {
            [self.delegate1 cancelBtnClick:titleAddress titleIDs:self.titleIDMarr];
        }
    }];
}
