...
//  HanPay
//
//  Created by 张海彬 on 2019/3/15.
//  Copyright © 2019年 张海彬. All rights reserved.
//

#import "TakeOut_H5.h"
#import <WebKit/WebKit.h>
@interface TakeOut_H5 ()<WKUIDelegate,WKScriptMessageHandler,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,WKNavigationDelegate,UIGestureRecognizerDelegate>
{
    UIView *maskView;
}
...
-(void)setupNavBar{
    ...
    self.customNavBar.barBackgroundImage = [UIImage new];
...
- (void)initWKWebView{
    ...
    self.webView = [[WKWebView alloc] initWithFrame:CGRectMake(0,-self.heightStatus, SCREEN_WIDTH, SCREEN_HEIGHT) configuration:configuration];
...
-(void)loadWKwebViewData{
    ...
    if ([NSString isNOTNull:self.urlString]) {
         url = [NSURL URLWithString:[@"http://192.168.0.112:8080/" stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
    }else{
...
-(void)pageJumpParameter:(NSDictionary *)dict{
    TakeOut_H5 *wkWebView = [[TakeOut_H5 alloc]init];
